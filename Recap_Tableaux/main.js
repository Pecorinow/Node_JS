...
tasks.push(taskToAdd); // On pourrait aussi juste ajouter l'objet complet, sans créer une variable avant, mais c'est plus propre avec une variable.

//? Parcourir : Boucles prévues pour parcourir les tableaux :

//* for ... of () : 
// :( pas accès à l'indice
for (let task of tasks) { // = à chaque tour de boucle, on ajoute una tâche
...
tasks.forEach ( (task, indice) => {
    console.log(task);
}); // Exactement le même résultat que le for of au-dessus.

// Le forEach ne marche pas toujours => for of à la place.
// Exemple : en fait non, elle n'en a pas donné -_-

    // Parenthèse : On peut créer une fonction de 2 manières:
    function direBonjour() {};
    // OU :
    const direBonjour = () => {}; // = créer une fonction vierge, qu'on peut remplir plus tard.
    // La 2e méthode permet de faire un truc cool mais j'ai po compris (? éviter de créer des fonctions qui ont le même nom ?).
...
// Attention : si plusieurs éléments correspindent à notre condition, la fonction finc renvoie toujours le premier élément qu'il trouve :
let taskFood = tasks.find((task) => task.category === 'Nourriture');
console.log(taskFood);
...
// Dans ce cas, on doit faire attention au genre de condition qu'on écrit (par exemple, si on crée un if(!adminTasks), le tableau existera, mais vide, donc la condition sera biaisée).
...
//* MAP :
let tasksV2 = tasks.map (task => task.name); // ici, le nouveau tableau devient juste un tableau de chaines de caractères avec le noim de tâches.
...
console.log(task1);
// Sera très utilisé en React surtout.
...
console.log(wordsFlop); // Le résultat est une string de tous les mots, collés et moches.

// Ou ça :
const wordsMiFlop = foodWords.concat(animalWords);
console.log(wordsMiFlop); // Ici, ça marche mais c'est chiant à écrire.
